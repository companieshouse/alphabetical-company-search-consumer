...
module "ecs-service-green" {
  source = "git@github.com:companieshouse/terraform-modules//aws/ecs/ecs-service?ref=1.0.294"
  count  = var.create_green_alphabetical_company_search_consumer_service ? 1 : 0
  ...
  # Disable cloudwatch alarms that are dependant on load balancer
  cloudwatch_unhealthy_host_count_enabled = false
  cloudwatch_healthy_host_count_enabled   = false
  cloudwatch_response_time_enabled        = false
  cloudwatch_http_5xx_error_count_enabled = false
  ...
  docker_registry   = var.docker_registry
  docker_repo       = local.docker_repo
  container_version = var.green_alphabetical_company_search_consumer_version
  container_port    = local.container_port
  ...
  task_environment          = local.task_environment
  task_secrets              = local.task_secrets
  app_environment_filename  = local.green_app_environment_filename
  use_set_environment_files = local.use_set_environment_files
}
